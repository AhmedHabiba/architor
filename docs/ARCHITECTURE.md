...
- **Phase Rules**: The four-phase sequence and what is/isn't allowed in each phase.
...
| Skill | Purpose |
|-------|---------|
| **architecture-methodology** | Orchestration: enforces phase sequence, blocks out-of-phase discussions |
...
| **state-manager** | Task: state.json read/write logic, valid transitions, component tracking |
...
Eleven user-invoked commands provide structured entry points to each phase:

**Phase commands**: `/analyze-prd`, `/propose-methodology`, `/design-component`, `/generate-docs`

**Decision commands**: `/accept`, `/refine`, `/alternative`

**Utility commands**: `/status`, `/decision-log`, `/review-component`, `/help`

Each command reads `state.json` before executing and validates prerequisites. For example, `/propose-methodology` checks that Phase 1 is accepted before proceeding.
...
- Checks prerequisites (e.g., Phase 1 must be accepted before Phase 2 begins)
...
1. **Evaluate** (Phase 1): Analyze the PRD — extract requirements, identify gaps, assess risks
2. **Decide** (Phase 2): Propose architecture pattern and holistic component overview
3. **Design** (Phase 3): Detail each component one at a time — technology choices, integration points, failure modes
4. **Document** (Phase 4): Consolidate everything into a comprehensive architecture document

### Gate Rules

- Phase 1 → Phase 2: PRD analysis must be explicitly accepted
- Phase 2 → Phase 3: Both architecture pattern AND component overview must be accepted
...
- Accepted components cannot be reverted
